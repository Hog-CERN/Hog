$( document ).ready(function() {
  ...
  document.getElementById("side-nav").classList.remove("ui-resizable") ;
  ...
  $('[id^="navrow"] > ul.tablist').each(
  function(){
    var btn_group = $('<div class="btn-group" role="group" aria-label="Button group"></div>');
    $(this).find("li").each(
    function() { 
      var btn = $('<a type="button" class="btn"></a>')
      btn.attr('href', $(this).find("a").attr('href'));
      btn.html( $(this).find("span").html());
      btn_group.append(btn);
    });
    left_nav.append(btn_group);
    $(this).remove();
  });
  var right_nav = $('<div id="right_nav" class="col-md-3"></div>').append('\
  ...
  $(nav_container).append(left_space);
