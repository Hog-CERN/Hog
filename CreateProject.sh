      ...
      if [ -z ${HOG_LIBPATH+x} ]; then
        if [ -z ${HOG_SIMULATION_LIB_PATH+x} ]; then
          if [ "$COMMAND" == "libero" ]; then
            Logger "${HDL_COMPILER}" "${COMMAND_OPT}"../Hog/Tcl/create_project.tcl "${POST_COMMAND_OPT}$VERBOSE $PROJ"
          else
            Logger ${HDL_COMPILER} ${COMMAND_OPT}../Hog/Tcl/create_project.tcl ${POST_COMMAND_OPT}$VERBOSE $PROJ
          fi
        else
          if [ "$COMMAND" == "libero" ]; then
            Logger "${HDL_COMPILER}" "${COMMAND_OPT}"../Hog/Tcl/create_project.tcl "${POST_COMMAND_OPT}-simlib_path ${HOG_SIMULATION_LIB_PATH} $VERBOSE $PROJ"
          else
            Logger ${HDL_COMPILER} ${COMMAND_OPT}../Hog/Tcl/create_project.tcl ${POST_COMMAND_OPT}-simlib_path ${HOG_SIMULATION_LIB_PATH} $VERBOSE $PROJ
          fi
        fi
        ...
        if [ "$COMMAND" == "libero" ]; then
          Logger "${HDL_COMPILER}" "${COMMAND_OPT}"../Hog/Tcl/create_project.tcl "${POST_COMMAND_OPT}-simlib_path ${HOG_LIBPATH} $VERBOSE $PROJ"
        else
          Logger ${HDL_COMPILER} ${COMMAND_OPT}../Hog/Tcl/create_project.tcl ${POST_COMMAND_OPT}-simlib_path ${HOG_LIBPATH} $VERBOSE $PROJ
        fi
      fi
