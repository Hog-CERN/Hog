...
Converting an existing project to Hog means to create the Hog list files and adapt your project to the Hog directory structure.
...
If you are migrating to Hog but you are not changing repository, you can follow the instructions below paying attention to execute them in a new branch.

Say you have a new repository called `new_repo` with url `new_repo_url`.
Your project named `fancy_project` is currently stored in some local folder `fancy_path`.

## Preliminary actions

First of all create a new folder where you will store your firmware and initialize it as a git repository:
...
To obtain this edit a file called .gitmodules with your favourete texrt editor (say emacs):

```console
  new_repo>emacs .gitmodules
```

Add the following content to the file, pay attention to the url that must be specified relatively to the path of your repository on gitlab.cern.ch[^a].
...
[^a]:This is done to make the Hog submodule inherit the protocol form the main repository. Thanks to this, whoever clones the repositor
