...
for HDL_COMPILER in "${COMPILERS_TO_CHECK[@]}"; do
    ...
    if ! select_command_from_line "$HDL_COMPILER"; then
...
if ! { [ -z ${EOS_MGM_URL+x} ] && [ -z ${HOG_OFFICIAL_BIN_EOS_PATH+x} ] && [ -z ${HOG_IP_EOS_PATH+x} ]; }; then
...
for compiler in "${COMPILERS_TO_CHECK[@]}"; do
    if [[ " ${compiler} " =~ "vivado" || " ${compiler} " =~ "planAhead" ]]; then
        echo --------------------------------
