  ...
  if {$what_to_do eq "push"} {
    ...
    if {$will_copy == 1} {
      ...
      if {[llength $ip_synth_files] > 0} {
        ...
        if [catch {exec xrdcp -r $xci_path  $::env(EOS_MGM_URL)//$ip_path/$file_name/} msg] {
        ...
        if [catch {exec xrdcp -r $ip_synth_files $::env(EOS_MGM_URL)//$ip_path/$file_name/synthesized} msg] {
          Msg Warning "Something went wrong when copying the IP files to EOS. Error message: $msg"
        }
      } else {
    ...
    if  {$ret != 0} {
      ...
      if  {($ret_g == 0) && ([llength $ip_gen_files] > 0)} {
        if [catch {exec xrdcp -r $::env(EOS_MGM_URL)//$ip_path/$file_name/generated/* $xci_path} msg] {
          Msg Warning "Something went wrong when copying the IP files to EOS. Error message: $msg"
        }
      } else {
      ...
      if  {($ret_s == 0) && ([llength $ip_syn_files] > 0)} {
        if [catch {exec xrdcp $::env(EOS_MGM_URL)//$ip_path/$file_name/synthesized/$xci_ip_name\_synth_1/*.rpt $xci_path} msg] {
          Msg Warning "Something went wrong when copying the IP files to EOS. Error message: $msg"
        }
      } else {
