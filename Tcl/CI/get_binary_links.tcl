...
foreach dir [glob -type d $repo_path/Top/* ] {
  ...
  if {"$ver"=="0" || "$ver"=="$tag" || $options(force)==1} {
    ...
    lassign [Execute curl {-s --request POST --header "PRIVATE-TOKEN: ${push_token}" --form "file=@$repo_path/${proj}-${ver}.zip" ${api}/projects/${proj_id}/uploads}] ret content 
    if {$ret != 0} {
    ...
    lassign [Execute curl {-s --header "PRIVATE-TOKEN: ${push_token}" "${api}/projects/${proj_id}/releases/$ver"}] ret msg 
    if {$ret != 0} {
