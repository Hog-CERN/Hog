...
if [ -z "$1" ]; then
  ...
  if [ "$LOAD" == "" ]; then
    ...
    echo "curl --request POST --header "PRIVATE-TOKEN: ${push_token}" --header "Content-Type: application/json" --data '{ "name": "'"$version_type"' version: '"$tag"'", "tag_name": "'"$tag"'", "description": "'"$DESC"'"}' $api/projects/${proj}/releases"
    curl --request POST --header "PRIVATE-TOKEN: ${push_token}" --header "Content-Type: application/json" --data '{ "name": "'"$version_type"' version: '"$tag"'", "tag_name": "'"$tag"'", "description": "'"$DESC"'"}' $api/projects/${proj}/releases
