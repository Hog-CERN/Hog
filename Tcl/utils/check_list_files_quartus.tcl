...
proc SplitLineQuoteSafe {line} {
  set words [split $line " "]
  set outValue
  set tmp 
  set i 0
  while {$i != [llength $words]} {
    lappend tmp [lindex $words $i]
    if {[regexp {\".+} $tmp]} {
      while { ![regexp {.+\"} $tmp] } {
        set i [expr {$i + 1}]
        lappend tmp [lindex $words $i]
    ...
    lappend outValue [join $tmp " "]
    set i [expr {$i + 1}]
  }
}

proc CheckQuartusList {project_name } {
  set project_path [file normalize "$repo_path/Projects/$project_name/"]
  set qsf_file_name [file normalize "$project_path/$project_name.qsf"]

  if { [is_project_open ] } {
    export_assignment
  }

  set qsf_file  [open "$qsf_file_name" r]
  set qsf_lines [split [read $qsf_file] "\n"]
  close $qsf_file
  set list_file_dictionary [dict create ]
  foreach line $qsf_lines {
    if {![regexp {^ *$} $line] & ![regexp {^ *\#} $line] } {
      set splitted_line [SplitLineQuoteSafe $line]
      if {[lindex splitted_line 0] == "set_global_assignment"} {
        set splitted_line [lreplace $splitted_line 0 0] 
        set prop_dictionary [dict create ]
        set props_w_args {"-name" "-library" "-comment" "-entity" "-section_id" "-tag"}
        set props_wo_args {"-disable" "-remove"}
        foreach prop props_w_args {
          set index [lsearch splitted_line $prop]
          if { $index != -1 } {
            set splitted_line [lreplace $splitted_line $index $index]
            dict append prop_dictionary $prop [lindex $splitted_line $index] 
            set splitted_line [lreplace $splitted_line $index $index] 
          } else {
            dict append prop_dictionary $prop "Default"
          }
        }
        foreach prop props_wo_args {
          set index [lsearch splitted_line $prop]
          if { $index != -1 } {
            set splitted_line [lreplace $splitted_line $index $index]
            dict append prop_dictionary $prop "True" 
          } else {
            dict append prop_dictionary $prop "False"
          }
        }
        if {[llength $splitted_line]==1} {
          dict append prop_dictionary "-value" [lindex $splitted_line 0]
        } else {
          
        }
        set name [dict get $prop_dictionary "-name"]
        if {[regexp {.+_FILE} $name]} {
          set library [dict get $prop_dictionary "-library"]
          if {[dict exists $list_file_dictionary $library]} {
            lappend [dict get $list_file_dictionary $library] [dict get $prop_dictionary "-value"]
          } else {
            dict create $list_file_dictionary $library {[dict get $prop_dictionary "-value"]} 
          }
        }
      }
    }
