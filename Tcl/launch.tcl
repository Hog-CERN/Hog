...
set default_commands {
  ...
  # OPTIONS: ext_path.arg, output.arg, compile_order, verbose
  }

  \^(QUESTASIM|QUEST)?$ {#
    set do_questasim 1
  # NAME: QUESTASIM or QUEST
  # DESCRIPTION: Create a Questasim project with the correct compile order for the Hog project.
  # OPTIONS: verbose
  }
...
set parameters {
  ...
  {compile_order "For hierarchy, generate a compile order file."}
...
if {$cmd == -1} {
  ...
  if {$do_hierarchy == 1} {
    ...
    Hierarchy $listProperties $listLibraries $repo_path $output_path $options(compile_order)
    exit 0
  }
  if {$do_sigasi == 1} {
    cd $repo_path
  ...
  if {$do_questasim == 1} {
    Msg Info "Creating Questasim project for $project_name..."
    set proj_dir $repo_path/Top/$project_name
    set proj_list_dir $repo_path/Top/$project_name/list

    # Check if hog.conf has #quest (optional for now)
    set conf_file $proj_dir/hog.conf
    if {![file exists $conf_file]} {
      Msg Error "hog.conf not found in $proj_dir"
      exit 1
    }
    set fp [open $conf_file r]
    set first_line [gets $fp]
    close $fp
    if {[string first "#quest" $first_line] == -1} {
      Msg Warning "Project $project_name does not have #quest as shebang in hog.conf. Found: $first_line"
      Msg Warning "Proceeding anyway for testing purposes."
    }
    ...
    set script_file "$repo_path/questasim_compile.do"
    set fp [open $script_file w]
    puts $fp "# Questasim compile script for $project_name"
    puts $fp "# Generated by Hog"
    puts $fp ""
    ...
    dict for {lib files} $listLibraries {
      foreach file $files {
        ...
        if {$ext == ".vhd" || $ext == ".vhdl"} {
          puts $fp "vcom -work $lib $file"
        } elseif {$ext == ".v" || $ext == ".sv"} {
          puts $fp "vlog -work $lib $file"
        }
      }
    }

    puts $fp ""
    puts $fp "# End of compile script"
    close $fp

    Msg Info "Questasim compile script created: $script_file"
    Msg Info "To use: vsim -do $script_file"
    exit 0
  }
